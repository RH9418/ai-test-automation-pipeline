...
#                 # 🔴 FIX: Flawless split that doesn't discard trailing empty columns
#                 parts = line.split("|")
#                 if len(parts) > 2:
#                     parts = parts[1:-1] # Safely drop the empty space before first pipe and after last pipe
...
#                         # Safely strip ** from headers regardless of inner spaces
#                         if "---" in desc and desc.startswith("**") and desc.endswith("**"):
...
#             # 🧠 Smart Chunk Caching (Only triggered if we actually need sections)
...
#     # --- PHASE 3 (Excel) ---
#     # Runs entirely independently of the chunking loop, processing the whole folder once.
#     if run_step_3:
...
# 🔴 Diagnostic Print: If you don't see this, the file isn't being run!
print("🚀 Initializing Artifact Generator...")
    ...
    for i, sec in enumerate(sections):
        ...
        for line in chunk_code.split('\n'):
            ...
            if "page.expect_download()" in line:
                master_pytest += f"{full_indent}{line_stripped.replace('page.expect_download()', 'safe_download(shared_page)')}\n"
                continue
    ...
    for md_file in os.listdir(docs_dir):
        ...
        for line in lines:
            ...
            if line.startswith("|"):
                ...
                # 🔴 FIX: Flawless array parsing completely immune to bracket stripping
                parts = line.split("|")
                if len(parts) > 2:
                    parts.pop(0)  # Drop leading empty element
                    parts.pop(-1) # Drop trailing empty element
                    parts = [p.strip() for p in parts]
                    
                    if len(parts) >= 3:
                        # Direct assignment with fallback handling
                        use_case = parts[0] if len(parts) > 0 else ""
                        desc = parts[1].replace("<br>", "\n").replace("<br/>", "\n") if len(parts) > 1 else ""
                        loc = parts[2] if len(parts) > 2 else ""
                        pf = parts[3] if len(parts) > 3 else ""
...
if __name__ == "__main__":
    ...
    # 🔴 FIX: Tuple bypasses the UI citation stripper!
    parser.add_argument('--step', type=int, choices=(1, 2, 3), help="Phase to run (1: Docs, 2: Pytest, 3: Excel). If omitted, runs all.")
    parser.add_argument('--files', nargs='*', help="Specific annotated script names to process.")
    args = parser.parse_args()
    ...
    if args.files:
        ...
        missing_files = [f for f in args.files if f not in available_files]
        if missing_files:
            missing_fname = missing_files.pop(0)
            print(f"\n⚠️ WARNING: Could not find '{missing_fname}' in {annotated_logs_dir}/.")
            print(f"Did you run 'python generate_annotations.py --files {missing_fname}' first?\n")
            sys.exit(1)
    else:
        scripts_to_process = [f for f in os.listdir(annotated_logs_dir) if f.endswith(".py")]
    ...
    # Only do File Chunking, Markdown, and Pytest if Step 1 or 2 is requested
    if run_step_1 or run_step_2:
        for script_name in scripts_to_process:
            print(f"\n{'='*60}\n🚀 Processing: {script_name}\n{'='*60}")
            script_path = os.path.join(annotated_logs_dir, script_name)
