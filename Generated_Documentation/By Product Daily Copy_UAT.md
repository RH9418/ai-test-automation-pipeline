...
| B.1 | Launch a Chromium browser instance in non-headless mode to begin the test. | `browser = playwright.chromium.launch(headless=False)` | | | |
| B.2 | Create a new browser context to ensure session data is isolated. | `context = browser.new_context()` | | | |
| B.3 | Open a new page within the created browser context to start interacting with the application. | `page = context.new_page()` | | | |
...
| I.1 | Click on the informational message that says 'Please note that a maximum of' to acknowledge it. | `page.get_by_text("Please note that a maximum of").click()` | | | |
| I.2 | Click on the adjustments icon to open the settings menu. | `page.locator(".pointer.zeb-adjustments").first.click()` | | | |
| I.3 | Click on 'Save Preference' to save the current column visibility settings. | `page.get_by_text("Save Preference").click()` | | | |
| I.4 | Click on the adjustments icon again to reopen the settings menu. | `page.locator(".pointer.zeb-adjustments").first.click()` | | | |
| I.5 | Click on 'Reset Preference' to reset the column visibility settings to default. | `page.get_by_text("Reset Preference").click()` | | | |
```
```markdown
...
```
```markdown
| | **--- FINAL COLUMN VISIBILITY TOGGLE ---** | | | | |
...
| P.2 | Click on the export icon to initiate the export process and trigger a file download. | `page.locator("div:nth-child(2) > #export-iconId > .icon-color-toolbar-active").click()` | | | |
```
```markdown
...
| S.1 | Select 'Product 1' from the list of available products by clicking on its name. | `page.get_by_text("Product 1").click()` | | | |
...
| S.3 | Click on the filter body wrapper to activate the filter input area. | `page.locator(".ag-filter-body-wrapper").click()` | | | |
```
```markdown
| | **--- FILTER OPERATOR SELECTION AND EXPLORATION ---** | | | | |
| T.1 | Open the filtering operator dropdown to view available filter conditions. | `page.get_by_role("combobox", name="Filtering operator").click()` | | | |
...
| W.4 | Click on the span element containing the text 'ARTESANO' to select the corresponding row. | `page.locator("span").filter(has_text=re.compile(r"^ARTESANO$")).click()` | | | |
| W.5 | Click on the text 'ARTESANO' to confirm or highlight the selection. | `page.get_by_text("ARTESANO").click()` | | | |
```
```markdown
| | **--- ROW SELECTION AND INTERACTION ---** | | | | |
| X.1 | Check the checkbox for the row containing 'ARTESANO' to toggle its selection. | `page.get_by_role("gridcell", name="Press Space to toggle row selection (unchecked)  ARTESANO").get_by_label("Press Space to toggle row").check()` | | | |
| X.2 | Double-click on the checkbox within the row to perform an additional action, possibly to confirm selection. | `page.locator(".ag-cell-value.ag-cell.ag-cell-not-inline-editing.ag-cell-normal-height.ag-cell-last-left-pinned.ag-column-first.no-border.ag-cell-focus > .ag-cell-wrapper > .ag-group-checkbox").dblclick()` | | | |
```
```markdown
| | **--- COLUMN HEADER INTERACTION AND SORTING ---** | | | | |
| Y.1 | Click on the text 'System Forecast Total (Plan' to interact with the column header or data. | `page.get_by_text("System Forecast Total (Plan").nth(3).click()` | | | |
| Y.2 | Click on the descending sort icon in the column header to sort the data in descending order. | `page.locator(".ag-cell-label-container.ag-header-cell-sorted-desc > .ag-header-cell-label > .ag-sort-indicator-container > .ag-sort-indicator-icon.ag-sort-descending-icon > .ag-icon").click()` | | | |
| Y.3 | Click on the ascending sort icon in the column header to sort the data in ascending order. | `page.locator(".ag-cell-label-container.ag-header-cell-sorted-asc > .ag-header-cell-label > .ag-sort-indicator-container > .ag-sort-indicator-icon.ag-sort-ascending-icon > .ag-icon").click()` | | | |
| Y.4 | Click on the header icon to interact with the column, possibly to open a menu or perform an action. | `page.locator(".ag-header-cell.ag-header-parent-hidden.ag-header-cell-sortable.ag-header-background.ag-focus-managed.ag-header-active > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon").click()` | | | |
```
```markdown
| | **--- FILTER MENU INTERACTION ---** | | | | |
| Z.1 | Click on the filter body wrapper to open the filter options. | `page.locator(".ag-filter-body-wrapper").click()` | | | |
| Z.2 | Click on the combobox labeled 'Filtering operator' to open the dropdown menu. | `page.get_by_role("combobox", name="Filtering operator").click()` | | | |
| Z.3 | Select the 'Equals' option from the dropdown menu. | `page.get_by_role("option", name="Equals").click()` | | | |
```
```markdown
| | **--- FILTER OPTION SELECTION ---** | | | | |
| [.1 | Click on the text 'Equals' to confirm the selection. | `page.get_by_text("Equals").click()` | | | |
| [.2 | Select the 'Does not equal' option from the dropdown menu. | `page.get_by_role("option", name="Does not equal").click()` | | | |
| [.3 | Click on the text 'Does not equal' to confirm the selection. | `page.get_by_text("Does not equal").click()` | | | |
| [.4 | Select the 'Greater than' option from the dropdown menu. | `page.get_by_role("option", name="Greater than", exact=True).click()` | | | |
| [.5 | Click on the text 'Greater than' to confirm the selection. | `page.get_by_text("Greater than").click()` | | | |
| [.6 | Click on the text 'Greater than or equal to' to confirm the selection. | `page.get_by_text("Greater than or equal to").click()` | | | |
| [.7 | Click on the text 'Greater than or equal to' again, possibly to confirm or toggle the selection. | `page.get_by_text("Greater than or equal to").click()` | | | |
| [.8 | Select the 'Less than' option from the dropdown menu. | `page.get_by_role("option", name="Less than", exact=True).click()` | | | |
| [.9 | Click on the text 'Less than' to confirm the selection. | `page.get_by_text("Less than").click()` | | | |
| [.10 | Select the 'Between' option from the dropdown menu. | `page.get_by_role("option", name="Between").click()` | | | |
| [.11 | Click on the text 'Between' to confirm the selection. | `page.get_by_text("Between").click()` | | | |
| [.12 | Select the 'Blank' option from the dropdown menu. | `page.get_by_role("option", name="Blank", exact=True).click()` | | | |
```
```markdown
| | **--- LOGICAL OPERATOR AND FILTER VALUE INPUT ---** | | | | |
| [.1 | Click on the text 'AND' to set the logical operator for the filter. | `page.get_by_text("AND", exact=True).click()` | | | |
| [.2 | Click on the text 'OR' to set the logical operator for the filter. | `page.get_by_text("OR", exact=True).click()` | | | |
| [.3 | Click on the spinbutton labeled 'Filter Value' to activate the input field. | `page.get_by_role("spinbutton", name="Filter Value").click()` | | | |
| [.4 | Fill the spinbutton labeled 'Filter Value' with the value '1'. | `page.get_by_role("spinbutton", name="Filter Value").fill("1")` | | | |
| [.5 | Click on the spinbutton labeled 'Filter Value' again, possibly to confirm the input. | `page.get_by_role("spinbutton", name="Filter Value").click()` | | | |
| [.6 | Click on the 'Apply' button to apply the filter settings. | `page.get_by_role("button", name="Apply").click()` | | | |
```
```markdown
...
```
```markdown
| | **--- RESET AND COLUMN VISIBILITY CONFIGURATION PART 1 ---** | | | | |
...
```
```markdown
| | **--- RESET AND COLUMN VISIBILITY CONFIGURATION PART 2 ---** | | | | |
...
| `.2` | Click on the export icon to initiate the export process. Locate the export icon in the toolbar and click it to trigger the file export. | `page.locator("div:nth-child(2) > #export-iconId > .icon-color-toolbar-active").click()` | | | |
...
| a.3 | Save the current preference settings by selecting the 'Save Preference' option. Look for the 'Save Preference' text and click it. | `page.get_by_text("Save Preference").click()` | | | |
...
| a.5 | Reset the preference settings to their default state by selecting the 'Reset Preference' option. Look for the 'Reset Preference' text and click it. | `page.get_by_text("Reset Preference").click()` | | | |
```
```markdown
...
```
```markdown
| | **--- CHECKBOX INTERACTION - INITIAL TOGGLING ---** | | | | |
...
| g.2 | Click on the first checkbox again to toggle or confirm the selection. | `page.locator(".d-flex.flex-column.justify-content-center.font-size-10.align-items-center.checkbox-v2.m-r-10.zeb-check").first.click()` | | | |
| g.3 | Click on the first checkbox once more to toggle or confirm the selection. | `page.locator(".d-flex.flex-column.justify-content-center.font-size-10.align-items-center.checkbox-v2.m-r-10.zeb-check").first.click()` | | | |
```
```markdown
...
```
```markdown
| | **--- DROPDOWN INTERACTION - DESELECT OR MODIFY ---** | | | | |
...
```
```markdown
| | **--- CHECKBOX INTERACTION - ADDITIONAL TOGGLING ---** | | | | |
...
```
```markdown
| | **--- MEASURE SELECTION - AGED NET AND SCAN UNITS ---** | | | | |
...
| r.1 | Click on the button within the 'Daily Summary Product:THOMAS' card to expand or interact with it. | `page.locator("esp-card-component").filter(has_text="Daily Summary Product:THOMAS").get_by_role("button").click()` | | | |
| r.2 | Click on the button within the 'Daily Summary Product:THOMAS' card again, possibly to toggle or confirm the action. | `page.locator("esp-card-component").filter(has_text="Daily Summary Product:THOMAS").get_by_role("button").click()` | | | |
```
```markdown
| | **--- TOGGLE INDIVIDUAL COLUMN VISIBILITY ---** | | | | |
| s.1 | Locate the '/01(Sun) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/01(Sun) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.2 | Locate the '/02(Mon) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/02(Mon) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.3 | Locate the '/03(Tue) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/03(Tue) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.4 | Locate the '/04(Wed) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/04(Wed) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.5 | Locate the '/05(Thu) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/05(Thu) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.6 | Locate the '/06(Fri) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/06(Fri) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.7 | Locate the '/07(Sat) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/07(Sat) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.8 | Locate the '/08(Sun) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/08(Sun) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.9 | Locate the '/09(Mon) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/09(Mon) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
...
| s.11 | Locate the '/11(Wed) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/11(Wed) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.12 | Locate the '/12(Thu) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/12(Thu) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
| s.13 | Locate the '/13(Fri) Column' and toggle its visibility to make it visible. | `page.get_by_role("treeitem", name="/13(Fri) Column").get_by_label("Press SPACE to toggle visibility (hidden)").check()` | | | |
```
```markdown
| | **--- TOGGLE ALL COLUMNS VISIBILITY ---** | | | | |
| t.1 | Check the 'Toggle All Columns Visibility' checkbox to make all columns visible. | `page.get_by_role("checkbox", name="Press SPACE to toggle visibility (hidden)").check()` | | | |
```
```markdown
...
| w.4 | Select the 'Reset Preference' option from the dropdown menu to reset the preferences to default. | `page.locator("div").filter(has_text=re.compile(r"^Reset Preference$")).first.click()` | | | |
```
```markdown
