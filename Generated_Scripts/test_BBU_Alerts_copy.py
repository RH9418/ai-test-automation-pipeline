...
def test_navigation_to_executive_dashboard(shared_page: Page):
    ...
    safe_action(shared_page, shared_page, 'goto', "Navigate to the specified URL for the 'Executive Dashboard' in the demand planning application.", "https://stage.bbu.esp.antuit.ai/dp/demand-planning/executive-dashboard?workbookId=4&tabIndex=1")
    print('''
    ================================================================================
...
@pytest.mark.order(4)
def test_filter_selection_workflow(shared_page: Page):
...
@pytest.mark.order(7)
def test_file_download_handling(shared_page: Page):
...
@pytest.mark.order(8)
def test_reset_preferences(shared_page: Page):
...
@pytest.mark.order(9)
def test_filter_operations(shared_page: Page):
...
def test_dropdown_status_selection(shared_page: Page):
...
def test_treegrid_navigation_and_context_menu_actions(shared_page: Page):
...
def test_grid_interaction_and_pagination(shared_page: Page):
...
def test_checkbox_operations(shared_page: Page):
...
def test_filter_columns_and_adjust_visibility(shared_page: Page):
...
def test_save_and_export_preferences(shared_page: Page):
    # Click on the 'Preference' dropdown menu to open the preference options.
    safe_action(shared_page, shared_page.locator("div:nth-child(3) > #preference-iconId > .legend-font > .multiselect-dropdown > .pointer"), 'click', "Click on the 'Preference' dropdown menu to open the preference options.")
    # Click on the 'Save Preference' option to save the current grid settings.
    safe_action(shared_page, shared_page.get_by_text("Save Preference"), 'click', "Click on the 'Save Preference' option to save the current grid settings.")
    ...
    # Click on the 'Export' button to export the grid data, triggering a file download.
        safe_action(shared_page, shared_page.locator("div:nth-child(2) > #export-iconId > .icon-color-toolbar-active"), 'click', "Click on the 'Export' button to export the grid data, triggering a file download.")
    # Store the downloaded file information for further validation or processing.
    download1 = download1_info.value

@pytest.mark.order(17)
def test_reset_preferences_and_apply_column_filter(shared_page: Page):
    # Click on the 'Preference' dropdown menu again to reopen the preference options.
    safe_action(shared_page, shared_page.locator("div:nth-child(3) > #preference-iconId > .legend-font > .multiselect-dropdown > .pointer"), 'click', "Click on the 'Preference' dropdown menu again to reopen the preference options.")
    # Click on the 'Reset Preference' option to revert the grid settings to their default state.
    safe_action(shared_page, shared_page.get_by_text("Reset Preference"), 'click', "Click on the 'Reset Preference' option to revert the grid settings to their default state.")
    # Click on the filter icon in the column header to open the filter options for the selected column.
    safe_action(shared_page, shared_page.locator(".ag-header-cell.ag-header-parent-hidden.ag-header-background.ag-focus-managed.ag-header-active > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon"), 'click', "Click on the filter icon in the column header to open the filter options for the selected column.")
    # Enter the filter value '761' into the filter input field to apply a filter to the column.
    safe_action(shared_page, shared_page.get_by_role("spinbutton", name="Filter Value"), 'fill', "Enter the filter value '761' into the filter input field to apply a filter to the column.", "761")
    # Click on the 'Apply' button in the column filter menu to apply the filter and update the grid data.
    safe_action(shared_page, shared_page.get_by_label("Column Filter").get_by_role("button", name="Apply"), 'click', "Click on the 'Apply' button in the column filter menu to apply the filter and update the grid data.")
    # Click on the filter icon again to reopen the column filter menu after applying the filter.
    safe_action(shared_page, shared_page.locator(".ag-header-icon.ag-header-cell-filter-button.ag-filter-active > .ag-icon"), 'click', "Click on the filter icon again to reopen the column filter menu after applying the filter.")
    ...
    safe_action(shared_page, shared_page.get_by_label("Column Filter").get_by_role("button", name="Reset"), 'click', "Click on the 'Reset' button in the column filter menu to clear the applied filter and restore the grid data to its original state.")

@pytest.mark.order(18)
def test_row_selection_and_drill_down(shared_page: Page):
...
def test_context_menu_actions_and_navigation(shared_page: Page):
...
def test_apply_filters_and_interact_with_filter_options(shared_page: Page):
...
def test_weekly_summary_customer_card_interaction(shared_page: Page):
...
def test_file_export_and_download(shared_page: Page):
...
def test_event_section_interaction(shared_page: Page):
...
def test_column_visibility_and_filtering(shared_page: Page):
...
def test_saving_preferences_and_exporting_data(shared_page: Page):
...
def test_pagination_navigation(shared_page: Page):
    # Click on the second page link in the pagination to navigate to the second page of the grid.
    safe_action(shared_page, shared_page.locator("a").nth(2), 'click', "Click on the second page link in the pagination to navigate to the second page of the grid.")
    # Click on the page link labeled '3' to navigate to the third page of the grid.
    safe_action(shared_page, shared_page.locator("a").filter(has_text="3"), 'click', "Click on the page link labeled '3' to navigate to the third page of the grid.")
    # Click on the 'First Page' button in the pagination controls to navigate back to the first page of the grid.
    safe_action(shared_page, shared_page.locator("div:nth-child(7) > div > esp-grid-container > esp-card-component > .card-container > .card-content > esp-row-dimentional-grid > div > #paginationId > esp-pagination-v2 > .d-flex.w-100 > .pagination-pager > .pagination-first"), 'click', "Click on the 'First Page' button in the pagination controls to navigate back to the first page of the grid.")

@pytest.mark.order(28)
def test_row_display_settings(shared_page: Page):
    # Click on the dropdown labeled 'View 10 row(s)' to open the row display options.
    safe_action(shared_page, shared_page.get_by_text("View 10 row(s)").nth(2), 'click', "Click on the dropdown labeled 'View 10 row(s)' to open the row display options.")
    # Select the option 'View 20 row(s)' from the dropdown to change the number of rows displayed.
    safe_action(shared_page, shared_page.locator("div").filter(has_text=re.compile(r"^View 20 row\(s\)$")).nth(1), 'click', "Select the option 'View 20 row(s)' from the dropdown to change the number of rows displayed.")

@pytest.mark.order(29)
def test_column_filtering_and_resetting(shared_page: Page):
    # Click on the column header icon to open the filter options for the column.
    safe_action(shared_page, shared_page.locator(".ag-header-cell.ag-header-parent-hidden.ag-header-cell-sortable.ag-header-background.ag-focus-managed.ag-header-active > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon"), 'click', "Click on the column header icon to open the filter options for the column.")
    # Fill the filter textbox with the value 'Promotion' to filter the column based on this value.
    safe_action(shared_page, shared_page.get_by_role("textbox", name="Filter Value"), 'fill', "Fill the filter textbox with the value 'Promotion' to filter the column based on this value.", "Promotion")
    # Click the 'Apply' button to apply the column filter.
    safe_action(shared_page, shared_page.get_by_label("Column Filter").get_by_role("button", name="Apply"), 'click', "Click the 'Apply' button to apply the column filter.")
    # Click on the filtered column header to interact with the column filter settings.
    safe_action(shared_page, shared_page.locator(".ag-header-cell.ag-header-parent-hidden.ag-header-cell-sortable.ag-header-background.ag-focus-managed.ag-header-cell-filtered > .ag-header-cell-comp-wrapper > .ag-cell-label-container"), 'click', "Click on the filtered column header to interact with the column filter settings.")
    # Click on the filter icon to open the active filter options for the column.
    safe_action(shared_page, shared_page.locator(".ag-header-icon.ag-header-cell-filter-button.ag-filter-active > .ag-icon"), 'click', "Click on the filter icon to open the active filter options for the column.")
    # Click the 'Reset' button to clear the applied column filter.
    safe_action(shared_page, shared_page.get_by_label("Column Filter").get_by_role("button", name="Reset"), 'click', "Click the 'Reset' button to clear the applied column filter.")

@pytest.mark.order(30)
def test_grid_cell_interaction(shared_page: Page):
