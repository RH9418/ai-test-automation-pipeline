...
def test_navigation_to_create_promotions_page(shared_page: Page):
    safe_action(
        shared_page.goto,
        "https://stage.mkr.esp.antuit.ai/nglcp/promotions/create-promotions",
...
def test_promotion_event_details_input(shared_page: Page):
    safe_action(
        shared_page.get_by_test_id("promotionEventName").click,
        None,
        "Click on the 'Name' input field to begin entering the promotion event name."
    )
    safe_action(
        shared_page.get_by_test_id("promotionEventName").fill,
        "rh--1137-0904",
        "Fill the 'Name' input field with the value 'rh--1137-0904'."
    )
    safe_action(
        shared_page.get_by_test_id("promotionEventDescription").click,
        None,
        "Click on the 'Description' input field to begin entering the promotion event description."
    )
    safe_action(
        shared_page.get_by_test_id("promotionEventDescription").fill,
        "PROMO BUNDLE TEST 2",
...
def test_additional_considerations_and_location_selection(shared_page: Page):
    safe_action(
        shared_page.get_by_test_id("isAsapPricing").click,
        None,
        "Click on the 'ASAP Pricing' checkbox to enable this option under 'Additional Considerations'."
    )
    safe_action(
        shared_page.get_by_role("textbox", name="Select Locations").click,
        None,
        "Click on the 'Select Locations' textbox to open the location selection panel."
    )
    safe_action(
        shared_page.locator("#SideFilterlocationhierarchyId").get_by_text("Hierarchy").click,
        None,
        "Click on 'Hierarchy' in the location selection panel to expand the hierarchy options."
    )
    safe_action(
        shared_page.get_by_text("Region", exact=True).click,
        None,
        "Select 'Region' from the hierarchy options."
    )
    safe_action(
        shared_page.get_by_role("radio", name="JPN").check,
        None,
        "Select the 'JPN' radio button to filter locations by the Japan region."
    )
    safe_action(
        shared_page.get_by_text("Price Zone Type").click,
        None,
        "Click on 'Price Zone Type' to expand the price zone type options."
    )
    safe_action(
        shared_page.locator(".filter-values.d-flex.align-items-center.p-l-32.p-r-24 > .custom-checkbox-wrapper > .pointer").first.click,
        None,
        "Select the first checkbox under 'Price Zone Type' (e.g., 'Select All')."
    )
    safe_action(
        shared_page.get_by_text("Price Zone", exact=True).click,
        None,
        "Click on 'Price Zone' to expand the price zone options."
    )
    safe_action(
        shared_page.locator(".filter-values.d-flex.align-items-center.p-l-32.p-r-24 > .custom-checkbox-wrapper > .pointer").click,
        None,
        "Select a specific price zone checkbox (e.g., 'Collection')."
    )
    safe_action(
        shared_page.get_by_role("button", name="Apply Filters").click,
        None,
...
def test_product_selection(shared_page: Page):
    safe_action(
        shared_page.get_by_role("textbox", name="Select Products").click,
        None,
        "Click on the 'Select Products' textbox to open the product selection panel."
    )
    safe_action(
        shared_page.locator("#SideFilterproducthierarchyId").get_by_text("Hierarchy").click,
        None,
        "Click on 'Hierarchy' in the product selection panel to expand the hierarchy options."
    )
    safe_action(
        shared_page.get_by_text("Style Color").click,
        None,
    ...
    safe_action(
        shared_page.locator("div:nth-child(5) > esp-filter-sub-accordion-v1 > .sub-accordion-element > .fiter-values-container > .filter-options > .filter-values-options > .filter-values.d-flex.align-items-center.p-l-32.p-r-12 > .custom-checkbox-wrapper").click,
        None,
        "Select the first checkbox under 'Style Color' (e.g., 'Select All') to include all style colors."
    )
    safe_action(
        shared_page.locator("div:nth-child(5) > esp-filter-sub-accordion-v1 > .sub-accordion-element > .fiter-values-container > .filter-options > .filter-values-options > div:nth-child(2) > .custom-checkbox-wrapper > .pointer").click,
        None,
        "Select a specific style color checkbox (e.g., '-2C8-01J999') to include it in the selection."
    )
    safe_action(
        shared_page.locator(".pointer.custom-checkbox-unchecked").first.click,
        None,
    ...
    safe_action(
        shared_page.locator("div").filter(has_text=re.compile(r'^101AKB0201$')).click,
        None,
...
def test_promotion_value_and_price_zone_adjustment(shared_page: Page):
    safe_action(
        shared_page.get_by_role("textbox", name="Add Value").click,
        None,
        "Click on the 'Add Value' textbox to input a value for the promotion."
    )
    safe_action(
        shared_page.get_by_role("textbox", name="Add Value").fill("35"),
        None,
...
def test_submission_and_approval_workflow(shared_page: Page):
    ...
    safe_action(
        shared_page.get_by_text("Reject").click,
        None,
    ...
    safe_action(
        shared_page.get_by_text("Withdraw", exact=True).click,
        None,
...
def test_final_steps_and_cleanup(shared_page: Page):
    safe_action(
        ...
        None,
        "Perform final cleanup steps: Click 'Next' to proceed, close the page, context, and browser to terminate the session."
    )
